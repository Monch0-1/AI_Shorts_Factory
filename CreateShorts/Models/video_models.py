...
class VideoRequest:
    ...
    def from_dict(cls, data: dict) -> 'VideoRequest':
        ...
        duration = options_data.get("duration_seconds") or data.get("duration_seconds", 60)
        v_index = options_data.get("video_index") or data.get("video_index")
        refiner = options_data.get("enable_refiner") or data.get("enable_refiner", False)
        # Handle rename from use_template to use_script_template
        template = options_data.get("use_script_template") or data.get("use_script_template") or data.get("use_template", False)
